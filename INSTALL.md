...
> 💡 **一键自动安装**（复制以下指令发给 Agent，即可自动完成自检 → 安装 → 验收）：
>
> 请执行 `python3 scripts/check_env.py` 环境自检；若有失败项，请按本文件 §2 安装清单与下方步骤自动完成安装（pip 依赖 / hooks 权限 / settings.json 权限路径 / 目录修复），并重跑验收直到 7 项全 PASS 且退出码 0。注意：Python 版本（编号 1）需用户手动安装，Agent 不可自动安装。
>
> 收到此指令即视为用户已授权一键执行；Python 版本失败时须停下并提示用户手动安装。
...
| 日期 | 版本 | 变更内容 | 同步载体 |
|------|------|----------|----------|
| 2026-08-11 | v1.1 | ①§3 新增「一键自动安装」提示框：一句可复制发给 Agent 的指令，一键完成自检 → 自动安装 → 验收（Python 除外），同步至 `操作使用说明书.html` | 本文件 + `操作使用说明书.html` |
